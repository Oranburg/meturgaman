...
Every path below is relative to this skill's own directory. Change into it
before running anything.

## First, one call, to learn whether this container has egress

```
python3 scripts/probe.py
```

It reports the Python version, whether the bundled package imports, and whether
each of the three hosts answers. Do this before promising anything, for one
specific reason.
...
This skill is written for a container that **can** reach those hosts, which on
claude.ai is a setting rather than a default. When the probe says BLOCKED, say so
plainly, give the user the one-time fix in `references/sandbox-on-claude-ai.md`,
and work the degraded path in `references/sefaria-fallback.md` under the rules in
`references/integrity-on-the-fallback.md` until they have made it.
...
it is standard library only, which is what lets it run where nothing can be
downloaded. Two behaviours to know: the CLI **exits 0 even when it refuses**, so
read the text rather than the exit code, and Hebrew should go in on standard
input where a command accepts `-`, because quoting Hebrew through a shell breaks
silently. Every command that talks to a service takes `--json` for structured
output and `--no-cache` to force a fresh fetch.
...
1. Topics beat search for anything anyone has thought about before:
       ...
       python3 scripts/mtg.py search "ribbit" --filter Halakhah

   `search` posts to Sefaria and so exists only with egress. Reach for it when
   no topic fits, and say it is unavailable rather than approximating it when
   the probe came back BLOCKED.
...
hand. Read `references/sefaria-api-traps.md` alongside it, because the spec
records signatures and that file records behaviour, and where they disagree the
...
and nothing on its face separates them.
    ...
    python3 scripts/mtg.py law hebrew remedies-1970     # the Hebrew, with its revision id
    python3 scripts/mtg.py law amendments remedies-1970 # which sections moved

The first three read bundled data and work with no network. The last two reach
he.wikisource.org, which the probe tests separately for exactly that reason.
...
## Files: what comes in, and what goes out

`law parse`, `law align`, `law reconcile` and `verify` all read files, and
`study --output` and `audio` write them. Nothing in this container reaches the
user's own disk, so both directions need a step the desktop tool does not.

**In.** The user attaches the file to the conversation and it arrives in the
container. Find it rather than assuming a path, and say which file you read.

**Out.** A file reaches the user only if it is left at the top level of
`$OUTPUT_DIR`, so write there, or copy there in the same command, and list the
directory afterwards so the capture is visible:

    python3 scripts/mtg.py study "Genesis 1" --tier file --output "$OUTPUT_DIR" && ls "$OUTPUT_DIR"

A file written anywhere else stays in the container. Never tell the user a file
is ready without having seen it in that listing.
...
Some things this skill cannot do here whatever the network setting, including
playing audio and adding vowel points with Dicta's models. What to offer instead
is in `references/capability-map.md`. Say what is unavailable plainly; do not
